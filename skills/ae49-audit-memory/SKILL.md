...
1. A table of each memory file with a status: **OK / stale / duplicate / misfiled**.
2. Suggestions grouped under **Keep · Update · Delete · Create**, one line of reasoning each.

Then **ask the user which suggestions to apply**. Apply changes only after explicit approval, one approved item at a time, and update `MEMORY.md` to match.
