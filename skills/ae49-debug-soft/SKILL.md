---
name: ae49-debug-soft
description: Four-mantra debugging discipline — reproduce, trace the fail path, falsify the hypothesis, cross-reference every breadcrumb. Recite the mantra block verbatim at the start of any debugging session, then apply the four steps in order before proposing any fix. This is the lightweight default for everyday bugs. Trigger on /ae49-debug-soft and proactively whenever debugging starts — user reports a bug, says something is broken/throwing/failing, asks to debug/diagnose/investigate an issue, or pastes a stack trace or error log. For hard or intermittent bugs that need a built repro/feedback-loop harness, performance regressions, regression tests, or a post-mortem, escalate to ae49-debug-hard.
---
