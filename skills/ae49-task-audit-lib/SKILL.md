...
5. **Report + pick scope (change nothing).** Format per the **`ae49-ref-report-format`** skill
   (plain English, emoji, per-finding **IDs** so the user can reference one back). Produce a
   short, scannable, **prioritized** list; give each finding an ID and these lines:
   ...
   Group by priority, ID'd per tier (e.g. `T1-1`, `T1-2`, `T2-1`):
   - 🔴 **Tier 1 (`T1-*`) — clear duplication** (biggest copy-paste, most likely to drift apart).
   - 🟡 **Tier 2 (`T2-*`) — smaller tidy-ups.**
   - ⚪ **Tier 3 (`T3-*`) — optional / low value.**

   Then **ask which findings to put in the plan** (by ID). If the codebase is already DRY, say
