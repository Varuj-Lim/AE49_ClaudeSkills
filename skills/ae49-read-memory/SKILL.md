...
3. **Recap the current memory**, grouped by type: **User · Feedback · Project · Reference**. Keep it concise and scannable.
4. **Highlight what's NEW or CHANGED** versus what this session previously had in context — call out added rules, edited facts, or removed entries, so the user can see what the other session changed. If you can't tell what changed, just present the current state and say so.

## Principle
- **Read-only.** This skill never edits memory — it only loads and reports it.
- To *clean up / fix* memory (merge duplicates, prune, propose edits), use the **`ae49-audit-memory`** skill instead.
