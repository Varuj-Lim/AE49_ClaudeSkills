---
name: ae49-task-management-talk
...
> *Adapted from [thananon/9arm-skills](https://github.com/thananon/9arm-skills) — original skill `management-talk`. Credit: [@thananon](https://github.com/thananon).*
...
2. **Produce the draft** as a single chat block, formatted as the channel would render it.
   Plain-English / readability conventions come from the **`ae49-ref-report-format`** skill —
   emoji are welcome where the channel uses them (Slack, standup); keep them sparse for formal
   email / JIRA.
3. **Ask where it goes:**
...
See [EXAMPLES.md](EXAMPLES.md) for the same bug rewritten across channels (JIRA comment, Slack post, standup note) — showing exactly what each channel keeps and drops.
