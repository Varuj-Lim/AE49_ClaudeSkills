...
2. **Filter.** Read each file's **Status** (the first word of the Status line) and
   ...
   there's nothing to implement (plan one with `/ae49-plan-feature`) and stop.
...
3. **Map collisions, then list + select.** *Collision-check FIRST, so every option is
   ...
   **(b) List + select.** Present the **Buildable** plans for the user to pick one. For
   ...
   colliding plan(s), their status, and the shared file(s), e.g. "⚠ collides with
   ...
   **bold**, a one-line gist (its Context or first line), and any `⚠ collides with …`
   warning. Example:

   ```
   Buildable plans:
   1. 📋 ✅ **unified-leave-table-format** — merge Orders + Approval into one shared table. ⚠ collides with *absent-policy* (In progress) on `types/order.ts`
   ...
   **Reuse** section — don't rewrite what exists. Stay surgical: only what the plan
   ...
   - **Pause for a manual action (On hold).** If a step needs the *user* to do
     ...
     **and** the Step number to resume from;
     ...
     `/ae49-implement-feature`, pick the plan, and step 5 flips it back to
