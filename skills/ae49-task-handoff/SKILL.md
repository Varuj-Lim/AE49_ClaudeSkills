...
- **Merge-in-flight guard (multi-session repos with a build-folder pool).** Before staging, confirm HEAD is the default branch — `git rev-parse --abbrev-ref HEAD`. If it reads `_merge_preview` (or any non-default branch), a merge is mid-flight: do NOT commit — write the handoff file, tell the user it's saved and the commit is held, and hold until the hub is back on the default branch. Never `git checkout` the hub yourself. (On a solo repo with no such pool, HEAD is always the default branch — this check just passes.)
